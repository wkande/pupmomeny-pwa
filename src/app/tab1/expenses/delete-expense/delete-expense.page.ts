...
export class DeleteExpensePage implements OnInit {
  ...
  async apply() {
    try{
        ...
        console.log('http://192.168.0.14:3000/expenses/'+this.category.id+'/expense_items/'+this.expense.id, {headers: headers} )
        var result = await this.http.delete('http://192.168.0.14:3000/expenses/'+this.category.id+'/expense-items/'+this.expense.id, {headers: headers} ).pipe(timeout(5000)).toPromise();
